...
public class WSS11Builder implements AssertionBuilder<OMElement> {
  ...
  public Assertion build(OMElement element, AssertionBuilderFactory factory) throws IllegalArgumentException {
    ...
    final Wss11 wss11 = new Wss11(SPConstants.SP_V12);

    final Policy policy = PolicyEngine.getPolicy(element.getFirstElement()).normalize(false);

    final Iterator<List<Assertion>> alternative = policy.getAlternatives();

    if (alternative.hasNext()) { // there should be max one alternative
      processAlternative(alternative.next(), wss11);
    }
  ...
  private void processAlternative(final List<Assertion> assertions, final Wss11 parent) {
