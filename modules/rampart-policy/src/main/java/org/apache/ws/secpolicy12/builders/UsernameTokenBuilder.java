...
public class UsernameTokenBuilder implements AssertionBuilder<OMElement> {
  ...
  public Assertion build(OMElement element, AssertionBuilderFactory factory) throws IllegalArgumentException {
    ...
    final UsernameToken usernameToken = new UsernameToken(SPConstants.SP_V12);

    final OMAttribute attribute = element.getAttribute(SP12Constants.INCLUDE_TOKEN);
    ...
    if (policyElement != null && policyElement.getQName().equals(org.apache.neethi.Constants.Q_ELEM_POLICY)) {
      ...
      if (alternatives.hasNext()) { // there should be max one alternative
        processAlternative(alternatives.next(), usernameToken);
      }
  ...
  private void processAlternative(final List<Assertion> assertions, final UsernameToken parent) {
