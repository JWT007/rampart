...
public class EncryptedPartsBuilder implements AssertionBuilder<OMElement> {
  ...
  public Assertion build(OMElement element, AssertionBuilderFactory factory) throws IllegalArgumentException {
    ...
    final SignedEncryptedParts signedEncryptedParts = new SignedEncryptedParts(false, SPConstants.SP_V12);

    final Iterator<OMElement> iterator = element.getChildElements();

    if (iterator.hasNext()) {
      while (iterator.hasNext()) {
        processElement(iterator.next(), signedEncryptedParts);
      }
    } else {
    ...
    if (isOptional != null) {
      signedEncryptedParts.setOptional(Boolean.parseBoolean(isOptional.getAttributeValue()));
    }
  ...
  private void processElement(OMElement element, SignedEncryptedParts parent) {
    ...
    if (SP12Constants.HEADER.equals(name)) {
      ...
      final OMAttribute nameAttribute = element.getAttribute(SPConstants.NAME);

      if (nameAttribute != null) {
        header.setName(nameAttribute.getAttributeValue());
      }

      final OMAttribute namespaceAttribute = element.getAttribute(SPConstants.NAMESPACE);
