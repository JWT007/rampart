...
public class TokenRequestDispatcherConfig {
  ...
  private final Map<String, String> issuers = new Hashtable<>();
  private final Map<String, String> validators = new Hashtable<>();
  private final Map<String, String> renewers = new Hashtable<>();
  private final Map<String, String> configFiles = new Hashtable<>();
  private final Map<String, OMElement> configElements = new Hashtable<>();
  private final Map<String, String> configParamNames = new Hashtable<>();
  ...
  private static void handleIssuers(OMElement configElem,
    ...
    for (Iterator<OMElement> issuerElems = configElem.getChildrenWithName(ISSUER);
      ...
      for (Iterator<OMElement> tokenTypes = element.getChildrenWithName(TOKEN_TYPE);
        ...
        if (!conf.issuers.containsKey(value)) {
  ...
  private static void handleValidators(OMElement configElem,
    ...
    for (Iterator<OMElement> validatorElems = configElem.getChildrenWithName(VALIDATOR);
      ...
      for (Iterator<OMElement> tokenTypes = element.getChildrenWithName(TOKEN_TYPE);
        ...
        if (!conf.validators.containsKey(value)) {
  ...
  private static void handleRenewers(OMElement configElem,
    ...
    for (Iterator<OMElement> renewerElems = configElem.getChildrenWithName(RENEWER);
      ...
      for (Iterator<OMElement> tokenTypes = element.getChildrenWithName(TOKEN_TYPE);
        ...
        if (!conf.renewers.containsKey(value)) {
  ...
  public TokenIssuer getIssuer(String tokenType) throws TrustException {
    ...
    String issuerClassName = this.issuers.get(tokenType);
  ...
  public TokenValidator getValidator(String tokenType) throws TrustException {
    ...
    String validatorClassName = this.validators.get(tokenType);
  ...
  public TokenRenewer getRenewer(String tokenType) throws TrustException {
    ...
    String renewerClassName = this.renewers.get(tokenType);
  ...
  /**
   * @param issuerClassName the issuer class-name
   * @return TokenIssuer the issuer
   */
  private TokenIssuer createIssuer(String issuerClassName) throws Exception {
    TokenIssuer issuer =
      (TokenIssuer) Loader.loadClass(this.getClass().getClassLoader(), issuerClassName)
  ...
  private TokenCanceler createCanceler(String cancelerClassName) throws Exception {
    TokenCanceler canceler =
      (TokenCanceler) Loader.loadClass(this.getClass().getClassLoader(), cancelerClassName)
                            .getDeclaredConstructor()
                            .newInstance();
  ...
  private TokenValidator createValidator(String validatorClassName) throws Exception {
    TokenValidator validator =
      (TokenValidator) Loader.loadClass(this.getClass().getClassLoader(), validatorClassName)
                             .getDeclaredConstructor()
                             .newInstance();
  ...
  private TokenRenewer createRenewer (String renewerClassName) throws Exception {
    TokenRenewer renewer =
      (TokenRenewer) Loader.loadClass(this.getClass().getClassLoader(), renewerClassName)
                           .getDeclaredConstructor()
                           .newInstance();
