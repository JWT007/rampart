...
public class RahasData {
  ...
  private void processWSS4JSecurityResults() throws TrustException {
    ...
    List<WSHandlerResult> results =
      (List<WSHandlerResult>) this.inMessageContext.getProperty(WSHandlerConstants.RECV_RESULTS);
  ...
  /**
   * Finds the KeySize and creates an empty ephemeral key.
   *
   * @throws TrustException on error
   */
  private void processKeySize() throws TrustException {
    OMElement keySizeElem =
      this.rstElement.getFirstChildWithName(new QName(this.wstNs,
                                                      RahasConstants.IssuanceBindingLocalNames.
                                                        KEY_SIZE));
    if (keySizeElem != null) {
  ...
  private void processClaims() {
    ...
    claimElem =
      this
        .rstElement
        .getFirstChildWithName(new QName(this.wstNs, RahasConstants.IssuanceBindingLocalNames.CLAIMS));

    if (claimElem != null){
      claimDialect = claimElem.getAttributeValue(new QName(this.wstNs, RahasConstants.ATTR_CLAIMS_DIALECT));
    }
