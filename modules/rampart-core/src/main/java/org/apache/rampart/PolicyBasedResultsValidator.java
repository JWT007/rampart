...
public class PolicyBasedResultsValidator implements ExtendedPolicyValidatorCallbackHandler {
  ...
  private static final Logger LOGGER = LogManager.getLogger(PolicyBasedResultsValidator.class);
  ...
  protected void validateEncrSig(ValidatorData data,List<WSEncryptionPart> encryptedParts,
    ...
    if(encr && encryptedParts.size() == 0) {
      ...
      for (WSSecurityEngineResult result : list) {
        List<WSDataRef> dataRefURIs = (List<WSDataRef>) result.get(WSSecurityEngineResult.TAG_DATA_REF_URIS);
  ...
  protected void handleSupportingTokens(List<WSSecurityEngineResult> results, SupportingToken suppTok) throws RampartException {
    ...
    for (Token token : tokens) {
      if (token instanceof UsernameToken) {
        ...
        if (samlResult == null) {
          LOGGER.debug("No signed SAMLToken found. Looking for unsigned SAMLTokens");
  ...
  protected void validateProtectionOrder(ValidatorData data, List<WSSecurityEngineResult> results)
    ...
    for (Integer sigEncrAction : sigEncrActions) {
      if (sigEncrAction == WSConstants.SIGN) {
        sigNotPresent = false;
      } else if (sigEncrAction == WSConstants.ENCR) {
    ...
    if(SPConstants.SIGN_BEFORE_ENCRYPTING.equals(protectionOrder)) {
      ...
      for (Iterator<Integer> iter = sigEncrActions.iterator();
           iter.hasNext() || !done;) {
      ...
      for (Integer sigEncrAction : sigEncrActions) {
        ...
        if (sigEncrAction == WSConstants.SIGN && !encrFound) {
          // We found SIGN and ENCR has not been found - break and fail
          break;
        }
        if (sigEncrAction == WSConstants.ENCR) {
          encrFound = true;
        } else if (encrFound) {
          //We have an ENCR action after sig
          done = true;
  ...
  protected List<Integer> getSigEncrActions(List<WSSecurityEngineResult> results) {
    List<Integer> sigEncrActions = new ArrayList<>();
  ...
  protected void validateEncryptedParts(ValidatorData data,
    ...
    List<WSDataRef> encrRefs = getEncryptedReferences(results);

    RampartPolicyData rpd = rmd.getPolicyData();

    // build the list of encrypted nodes based on the dataRefs xpath expressions
    SOAPEnvelope envelope = rmd.getMsgContext().getEnvelope();
    Set<OMNamespace> namespaces = RampartUtil.findAllPrefixNamespaces(envelope,
                                                                      rpd.getDeclaredNamespaces());

    Map<OMElement, Boolean> decryptedElements = new HashMap<>();
    for (WSDataRef encrRef : encrRefs) {

      if (encrRef == null || encrRef.getXpath() == null) {
        continue;
      }

      try {
        XPath xp = new AXIOMXPath(encrRef.getXpath());

        for (OMNamespace namespaceObject : namespaces) {
          xp.addNamespace(namespaceObject.getPrefix(), namespaceObject.getNamespaceURI());
        }

        for (OMElement o : (List<OMElement>) xp.selectNodes(envelope)) {
          decryptedElements.put(o, encrRef.isContent());
        }
    ...
    for (WSEncryptionPart encryptedPart : encryptedParts) {
      ...
      try {
        ...
        for (OMElement node : (List<OMElement>) xp.selectNodes(envelope)) {
          Object result = decryptedElements.get(node);
          if (result != null &&
  ...
  protected void validateSignedPartsHeaders(ValidatorData data, List<WSEncryptionPart> signatureParts,
    ...
    List<QName> actuallySigned = new ArrayList<>();
    if (actionResults != null) {
      for (WSSecurityEngineResult actionResult : actionResults) {

        List<WSDataRef> wsDataRefs = (List<WSDataRef>) actionResult.get(WSSecurityEngineResult.TAG_DATA_REF_URIS);
  ...
  protected boolean isCertificateTrusted(
    ...
    LOGGER.debug(
      "Transmitted certificate has issuer {} (serial {}).",
      () -> issuerString,
      () -> issuerSerial);
    ...
    //
    LOGGER.debug("Preparing to validate certificate path for issuer {}.", () -> issuerString);
    ...
    try {
      if (crypto.verifyTrust(x509certs, false)) {
        LOGGER.debug("Certificate path has been verified for certificate with subject {}", () -> subjectString);
        return true;
      }
    } catch (WSSecurityException e) {
    ...
    LOGGER.debug("Certificate path could not be verified for certificate with subject {}.", () -> subjectString);
  ...
  protected boolean isCertificateInKeyStore(
    ...
    if (foundCerts != null && foundCerts[0] != null && foundCerts[0].equals(cert)) {
      ...
      LOGGER.debug("Direct trust for certificate with {}",
      ...
      LOGGER.debug("No certificate found for subject from issuer with {} (serial {})",
                   () -> issuerString,
                   () -> issuerSerial);
      return false;
  ...
  private boolean isRefIdPresent(List<WSDataRef> refList , String id) {
  ...
  public static WSSecurityEngineResult[] fetchActionResults(List<WSSecurityEngineResult> wsSecurityEngineResults, int action) {
    List<WSSecurityEngineResult> wsResult = new ArrayList<>();
    ...
    for (WSSecurityEngineResult wsSecurityEngineResult : wsSecurityEngineResults) {
      ...
      int resultAction = (Integer) wsSecurityEngineResult.get(WSSecurityEngineResult.TAG_ACTION);
      if (resultAction == action) {
        wsResult.add(wsSecurityEngineResult);
      }
    }

    return wsResult.toArray(new WSSecurityEngineResult[0]);

  }

  private boolean isRefIdPresent(List<WSDataRef> refList , QName qname) {
