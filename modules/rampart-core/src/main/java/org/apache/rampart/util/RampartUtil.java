...
public class RampartUtil {
  ...
  private static final Map<String, CachedCrypto> cryptoStore = new ConcurrentHashMap<>();
  ...
  public static CallbackHandler getPasswordCB(MessageContext msgContext, RampartPolicyData rpd) throws RampartException {
    ...
    if (rpd.getRampartConfig() != null && rpd.getRampartConfig().getPwCbClass() != null) {
      ...
      try {
        cbHandler = (CallbackHandler) cbClass.getDeclaredConstructor().newInstance();
  ...
  public static KerberosTokenDecoder getKerberosTokenDecoder(MessageContext msgContext, KerberosConfig kerberosConfig) throws RampartException {
    ...
    try {
      kerberosTokenDecoder = (KerberosTokenDecoder) krbTokenDecoderClass.getDeclaredConstructor().newInstance();
    } catch (java.lang.Exception e) {
  ...
  public static PolicyValidatorCallbackHandler getPolicyValidatorCB(MessageContext msgContext, RampartPolicyData rpd) throws RampartException {
    ...
    if (rpd.getRampartConfig() != null && rpd.getRampartConfig().getPolicyValidatorCbClass() != null) {
      ...
      try {
        cbHandler = (PolicyValidatorCallbackHandler) cbClass.getDeclaredConstructor().newInstance();
      } catch (java.lang.Exception e) {
  ...
  public static RampartConfigCallbackHandler getRampartConfigCallbackHandler(MessageContext msgContext,
    ...
    if (rpd.getRampartConfig() != null && rpd.getRampartConfig().getRampartConfigCbClass() != null) {
      ...
      try {
        rampartConfigCB = (RampartConfigCallbackHandler) cbClass.getDeclaredConstructor().newInstance();
      } catch (java.lang.Exception e) {
  ...
  private static Crypto createCrypto(Properties properties, ClassLoader classLoader) throws RampartException {
    ...
    try {
      ...
      LOGGER.error("Error loading crypto properties.", e);
   ...
   * @param issuerAddress the issuer address OM element
   ...
   * @param secConvTok the secure-conversation token
   * @return Return the SecurityContextidentifier of the token
   * @throws TrustException if a trust error occurs
   * @throws RampartException if an error occurs
   */
   ...
   * Request a token.
   * @param rmd the rampart message-data
   * @param rstTemplate the request template
   * @param issuerEpr the issuer address
  ...
  public static List<WSEncryptionPart> getSignedParts(RampartMessageData rmd) {
    ...
    if (rpd.isSignAllHeaders()) {
      Iterator<OMElement> childHeaders = envelope.getHeader().getChildElements();
      while (childHeaders.hasNext()) {
        OMElement hb = childHeaders.next();
  ...
  {
    Set<OMNamespace> results = new HashSet<>();

    //Find declared namespaces
    findPrefixNamespaces(currentElement,results);

    //Get all default namespaces
    List<OMNamespace> defaultNamespaces = getDefaultPrefixNamespaces(currentElement.getOMFactory());
  ...
  private static void findPrefixNamespaces(OMElement e, Set<OMNamespace> results) {
    ...
    Iterator<OMNamespace> iterator = e.getAllDeclaredNamespaces();
    ...
    Iterator<OMElement> children = e.getChildElements();

    while (children.hasNext()) {
      findPrefixNamespaces(children.next(), results);
    }
  }

  private static List<OMNamespace> getDefaultPrefixNamespaces(OMFactory factory)
  {
    List<OMNamespace> namespaces = new ArrayList<>();
  ...
  public static List<WSEncryptionPart> getContentEncryptedElements (List<WSEncryptionPart> encryptedPartsElements,
                                                                    SOAPEnvelope envelope,List<String> elements,
                                                                    Map<String, String> decNamespaces ) {

    Set<OMNamespace> namespaces = findAllPrefixNamespaces(envelope, decNamespaces);
    ...
    for (String expression : elements) {
      try {
        ...
        List<OMElement> selectedNodes = xp.selectNodes(envelope);
        ...
        for (OMElement selectedNode : selectedNodes) {
          ...
          OMAttribute wsuIdAttribute = selectedNode.getAttribute(new QName(WSConstants.WSU_NS, "Id"));
  ...
  public static List<WSEncryptionPart> getPartsAndElements(boolean sign, SOAPEnvelope envelope, boolean includeBody,
    ...
    List<OMElement> found = new ArrayList<>();
    ...
    for (WSEncryptionPart part : parts) {
      if (part.getName() == null) {
        ...
        Iterator<SOAPHeaderBlock> headerList = header.getHeaderBlocksWithNamespaceURI(part.getNamespace());

        while (headerList.hasNext()) {

          SOAPHeaderBlock shb = headerList.next();
  ...
  public static boolean checkRequiredElements(SOAPEnvelope envelope, Map<String, String> decNamespaces, String expression) {
    ...
    SOAPHeader header = envelope.getHeader();
    Set<OMNamespace> namespaces = findAllPrefixNamespaces(header, decNamespaces);
  ...
  public static Map<String, String> getContextMap(MessageContext msgContext) {
    //Fist check whether it is there
    Hashtable<String, String> contextMap =
      (Hashtable<String, String>) msgContext.getConfigurationContext()
                                            .getProperty(ConversationConstants.KEY_CONTEXT_MAP);
    ...
    if (contextMap == null) {
      ...
      msgContext.getConfigurationContext().setProperty(
        ConversationConstants.KEY_CONTEXT_MAP, contextMap);
    }
   ...
   * @param results the WS-handler results
   * @return the username
   */
  ...
  public static String getSigElementId(RampartMessageData rmd) {
    ...
    Iterator<SOAPHeaderBlock> secHeaders = header.getHeaderBlocksWithNamespaceURI(WSConstants.WSSE_NS);

    if (secHeaders != null && secHeaders.hasNext()) {
      SOAPHeaderBlock firstSecHeader = secHeaders.next();
  ...
  public static String getSAML10AssertionNamespace() {
    ...
    return WSConstants.SAMLTOKEN_NS + "#" + WSConstants.SAML_ASSERTION_ID;
  ...
  private static void setCryptoProperty(MessageContext msgContext, String signaturePropertyName,
    ...
    if (msgContext.getProperty(cryptoPropertyName) == null) {
      ...
      if (signaturePropertyFile == null) {
        ...
        LOGGER.debug("Signature crypto property file is not set. Property file key - "
  ...
  public static boolean isSecurityFault(SOAPFault fault) {
    ...
    if (code == null) {
      ...
      return subCode != null && isSecurityFaultCode(subCode);
    }
  }

  private static boolean isSecurityFaultCode(SOAPFaultClassifier code) {
    QName value = code.getValueAsQName();
    return value != null && value.getNamespaceURI().equals(WSConstants.WSSE_NS);
  }
  ...
  public static Collection<UsernameToken> getUsernameTokens(RampartPolicyData rpd) {
    Collection<UsernameToken> usernameTokens = new ArrayList<>();
