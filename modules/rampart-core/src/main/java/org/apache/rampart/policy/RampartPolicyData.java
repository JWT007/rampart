...
public class RampartPolicyData {
  ...
  private List<WSEncryptionPart> signedParts = new ArrayList<>();

  private List<String> signedElements = new ArrayList<>();

  private List<WSEncryptionPart> encryptedParts = new ArrayList<>();

  private List<String> encryptedElements = new ArrayList<>();

  private List<String> requiredElements = new ArrayList<>();

  private List<String> contentEncryptedElements = new ArrayList<>();
  ...
  public String getSupportingTokenID(Token token, int type)
    ...
    switch (type) {
      case SPConstants.SUPPORTING_TOKEN_SUPPORTING:
        if(this.supportingTokensIdMap != null) {
          return this.supportingTokensIdMap.get(token);
        }
        return null;

      case SPConstants.SUPPORTING_TOKEN_SIGNED:
        if(this.signedSupportingTokensIdMap != null) {
          return this.signedSupportingTokensIdMap.get(token);
        }
        return null;

      case SPConstants.SUPPORTING_TOKEN_ENDORSING:
        if(this.endorsingSupportingTokensIdMap != null) {
          return this.endorsingSupportingTokensIdMap.get(token);
        }
        return null;
  ...
  public boolean isMTOMSerialize(){
    if(mtomAssertion == null){
      ...
      return !mtomAssertion.isOptional();
    }
  }
