...
public class CryptoConfigBuilder implements AssertionBuilder<OMElement> {
  ...
  public Assertion build(OMElement element, AssertionBuilderFactory factory)
    ...
    cryptoConfig.setProvider(attribute.getAttributeValue().trim());

    OMAttribute cryptoKeyAttr = element.getAttribute(new QName(CryptoConfig.CRYPTO_KEY_ATTR));
    if(cryptoKeyAttr != null){
      cryptoConfig.setCryptoKey(cryptoKeyAttr.getAttributeValue().trim());
    }
    ...
    if(cacheRefreshIntAttr != null){
      cryptoConfig.setCacheRefreshInterval(cacheRefreshIntAttr.getAttributeValue().trim());
    }

    OMAttribute enableCryptoCacheAttr = element.getAttribute(new QName(CryptoConfig.CACHE_ENABLED));
    if(enableCryptoCacheAttr != null){
      cryptoConfig.setCacheEnabled(Boolean.parseBoolean(enableCryptoCacheAttr.
    ...
    for (Iterator<OMElement> iterator = element.getChildElements(); iterator.hasNext();) {
      /*
       * In this scenario we could have used
